...
kolejnosc = MatrixSegregation.sort_matrix_areas(macierz_przed, punkty, 10, 0, 0, list_of_lists=True)
# plt.plot([punkty[i][0] for i in kolejnosc], [punkty[i][1] for i in kolejnosc])
for a in range(1, len(kolejnosc)):
    plt.plot([punkty[i][0] for i in kolejnosc[a]], [punkty[i][1] for i in kolejnosc[a]])
