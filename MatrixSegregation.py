...
def make_cost_matrix(amount_of_points: int, amount_trucks: int, function_id: int = 1, max_val: int = 100, min_val: int = -100):
    points = random_2dim_points(amount_of_points, max_val, min_val)  # Losowanko punktow
    cost_matrix = create_cost_matrix(points)  # stworzenie macierzy kosztow
    if function_id == 1:
        cost_matrix, route = sort_matrix_by_distance(cost_matrix, 0)
    ...
    # Macierz kosztow, lista punktow (x, Y),
    return cost_matrix, points, route
>>>>>>> 42c3cb94d1b3496625dd4008fceaedefc6b5f271
